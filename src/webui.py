    ...
    if not os.path.exists(model_path):
        ...
        if pth_files:
            ...
            raise FileNotFoundError(
                f"No .pth file found in RVC model directory: {model_dir}"
            )
...
def voice_conversion(
    ...
    try:
        hubert_model = load_hubert(
            device, is_half, os.path.join(rvc_models_dir, "hubert_base.pt")
        )
        cpt, version, net_g, tgt_sr, vc = load_rvc_model(rvc_model)

        output_filename = os.path.join(
            output_dir, f"converted_{os.path.basename(input_audio)}"
        )
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="RVC Voice Changer") as app:
        with gr.Tab("Convert Voice"):
            with gr.Row():
                with gr.Column():
                    ...
                    gr.Markdown(
                        "Select the AI voice model you want to use for conversion. Models are stored in the 'rvc_models' folder."
                    )
                ...
                with gr.Column():
                    ...
                    gr.Markdown(
                        "Adjust the pitch of the output voice. Higher values make the voice higher, lower values make it deeper."
                    )
                    ...
                    gr.Markdown(
                        "Choose the algorithm for pitch detection. RMVPE is generally good for clarity, while Mangio-Crepe can produce smoother vocals."
                    )

                    index_rate = gr.Slider(0, 1, value=0.5, label="Index Rate")
                    gr.Markdown(
                        "Controls how much of the AI voice's characteristics to keep. Higher values result in output closer to the AI voice."
                    )
                    ...
                    gr.Markdown(
                        "Applies median filtering to pitch results. Can help reduce breathiness. Higher values smooth out the pitch more."
                    )

                    rms_mix_rate = gr.Slider(0, 1, value=0.25, label="RMS mix rate")
                    gr.Markdown(
                        "Controls how much to mimic the original vocal's volume envelope. Higher values preserve more of the original dynamics."
                    )

                    protect = gr.Slider(0, 0.5, value=0.33, label="Protect rate")
                    gr.Markdown(
                        "Protects voiceless consonants and breath sounds from being converted. Set to 0.5 to disable protection."
                    )
        ...
        with gr.Tab("Download model"):
            ...
            gr.Markdown(
                "Provide a direct download link to a zip file containing the voice model. The zip should include a .pth model file and an optional .index file."
            )

            model_name = gr.Text(label="Name your model")
            gr.Markdown(
                "Give your new model a unique name. This will be used as the folder name for the model files."
            )
        ...
        with gr.Tab("Upload model"):
            ...
            gr.Markdown(
                "Upload a zip file containing your voice model. The zip should include a .pth model file and an optional .index file."
            )

            upload_model_name = gr.Text(label="Model name")
            gr.Markdown(
                "Give your uploaded model a unique name. This will be used as the folder name for the model files."
            )
            ...
            upload_btn.click(
                upload_local_model,
                inputs=[upload_zip, upload_model_name],
                outputs=upload_output_message,
            )
